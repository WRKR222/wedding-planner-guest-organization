...
// (architecture §2/§13, NFR3/FR36). Resolves to exactly one wedding_id and
// can never be escalated to planner access or any other wedding's data.
async function requireCoupleSession(req) {
  ...
  if (!wedding || !wedding.couple_site_enabled) return null; // FR29e: disabling revokes access
  const { data: access } = await admin().from('couple_site_access').select('*').eq('wedding_id', wedding.id).maybeSingle();
  if (!access || access.revoked) return null; // FR36: revoking kills access immediately
