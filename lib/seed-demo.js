...
// Paginates auth.admin.listUsers() looking for the demo email. Needed when
// createUser() reports "already registered" but there's no planner_profiles
// row for it — e.g. someone added the user by hand in Supabase Studio
// (Authentication → Users) instead of through the app, which leaves an
// auth.users row with no profile and an unknown password.
...
async function seedDemo(db) {
  ...
  if (existingProfile) {
    ...
    if (error) {
      const existingAuthUser = await findAuthUserByEmail(db, DEMO_EMAIL);
      if (!existingAuthUser) throw error;
      plannerId = existingAuthUser.id;
    } else {
  ...
  // Whatever state the auth account was already in — a fresh create, an
  // existing one from a prior run, or one added by hand with some other
  // password — guarantee the documented demo credentials actually work.
  const { error: resetErr } = await db.auth.admin.updateUserById(plannerId, { password: DEMO_PASSWORD, email_confirm: true });
